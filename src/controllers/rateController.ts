...
class RateController {
  ...
  static async getFilteredDBRATES(req: Request, res: Response) {
    try {
      ...
      // Fetch all rates from the database
      const rates = await getRatesFromDBWithDateFilter(
        fromDate as string,
        toDate as string,
        pair as string,
        +page! as number
  ...
  static async getRatesAnalysis(req: Request, res: Response) {
    ...
    if (!currency || !startDate || !endDate) {
      return res
        .status(400)
        .json({ message: "Please provide currency, startDate, and endDate" });
    }
    try {
      const analysis = await getAnalyzedRates(
        currency as string,
        startDate as string,
        endDate as string
      );
