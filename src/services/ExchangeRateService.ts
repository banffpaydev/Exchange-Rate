...
import {
  ...
  central_reduct,
  inversePair,
} from "../controllers/treps";
import {
  createCurrencyPair,
  createRawCurrencyPair,
  getAdditionalRates,
  getAdditionalRatesId,
  getAllCurrencyPairs,
...
import {
  getPairById,
  updateRemitOneRate,
} from "../controllers/currencyPairController";
import InternalRate from "../models/internalRate";
import {
  autoUpdateInternalRatesOnFetch,
...
const afriXchangeRate = async (to: string, from: string) => {
  ...
  try {
    ...
    const response = await axios.get(
      `https://client.africhange.com/api/Rate?sendingCurrencyCode=${to}&receivingCurrencyCode=${from}`
    );
    const data = response.data.data;
...
export const xeRates = async (from: string, to: string) => {
  ...
  try {
    ...
    return {
      name: "Xe Exchange",
      rate: Number(ratings.rate),
      rawRate: Number(ratings.rawRate),
    };
  } catch (err: any) {
    // //console.error('Error fetching Afri Exchange rate:', err);
    return { msg: "error message", err };
...
const remitlyRate = async (from: string, to: string) => {
  try {
    const response = await axios.get(
      `https://api.remitly.io/v3/calculator/estimate?conduit=USA%3AUSD-NGA%3ANGN&anchor=SEND&amount=&purpose=OTHER&customer_segment=UNRECOGNIZED&strict_promo=false`
    );
    const data = response.data.data;
...
const cadrRemitRate = async (from: string, to: string) => {
  ...
  try {
    const response = await axios.get(
      `https://api.cadremit.com/v1/admin/settings`
    );
    const data = response.data.data;
    ...
    return {
      name: "CadRemit Exchange",
      rate: getRateD(rates, from, to),
      rawRate: getRateD(rates, from, to),
    };
  } catch (err: any) {
  ...
  try {
    ...
    const fromCountry = await ExchangeCountries.findOne({
      where: sequelize.where(
        sequelize.fn("LOWER", sequelize.col("currency")),
        from.toLowerCase()
      ),
    });
    ...
    if (fromCountry && toCountry) {
      const response = await axios.get(
        `https://app.sendwave.com/v2/pricing-public?amountType=SEND&receiveCurrency=${to}&amount=1&sendCurrency=${from}&sendCountryIso2=${
          from === "USD"
            ? "us"
            : from === "GBP"
            ? "gb"
            : fromCountry.code.toLowerCase()
        }&receiveCountryIso2=${
          to === "USD"
            ? "us"
            : to === "GBP"
            ? "gb"
            : toCountry.code.toLowerCase()
        }`
      );
      const data = response.data;
      return {
...
const wiseRate = async (from: string, to: string) => {
  ...
  try {
    const response = await axios.get(
      `https://wise.com/rates/live?source=${from}&target=${to}&length=30&resolution=hourly&unit=day`
    );
    const data = response.data;
...
const transfergoRate = async (from: string, to: string) => {
  ...
  try {
    ...
    const response = await axios.get(
      `https://my.transfergo.com/api/fx-rates?from=${Tfrom}&to=${Tto}&amount=1000`
    );
    const data = response.data;
    const vape = Number(data.rate);
...
const twelveDataRate = async (from: string, to: string) => {
  try {
    const response = await axios.get(
      `https://api.twelvedata.com/exchange_rate?symbol=${from}/${to}&apikey=${process.env.TWELVEDATA_API_KEY}`
    );
    const data = response.data;
...
const alphaVantageRate = async (from: string, to: string) => {
  try {
    const response = await axios.get(
      `https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=${from}&to_currency=${to}&apikey=${process.env.ALPHA_VANTAGE_KEY}`
    );
    const data = response.data;
    const treansferty =
...
const xchangeRtOrgRate = async (from: string, to: string) => {
  try {
    const response = await axios.get(
      `https://www.exchange-rates.org/api/v2/rates/lookup?isoTo=${to}&isoFrom=${from}&amount=1&pageCode=Home`
    );
    const data = response.data;
  ...
  try {
    ...
    for (const [, rates] of responseEntries) {
      const targetRate = rates.find(
        (rate) => rate.currency_name === (from === "NGN" ? to : from)
      );
...
export const handleAllFetch = async () => {
  ...
  const excludedNames = [
    "abokifxng",
    "cadrRemitRate",
    "Twelve Data Exchange",
    "Alphatvantage Exchange",
    "xchangeRt exchange",
  ];
  ...
  for (const pair of pairs) {
    ...
    try {
      ...
      await createCurrencyPair({
        ...pairData,
        exchangeRate: rate.banffPayRate,
...
export const getRatesFromDBWithDateFilter = async (
  startDate?: string,
  endDate?: string,
  pair?: string,
  page: number = 1,
...
export function getTopAndBottomRatesWithAverages(rates: VendorRate[]): {
  ...
  const uniqueRates = sortedRates.filter(
    (item, index, arr) => index === 0 || item.rate !== arr[index - 1].rate
  );
...
export const getAnalyzedRates = async (
  currency: string,
  startDate: string,
  endDate: string
) => {
  ...
  const additionalRates = await getAdditionalRatesId(
    currency_split[0].toUpperCase(),
    currency_split[1].toUpperCase()
  );
    ...
    ([pair, vendors]) => {
      ...
      return Object.entries(vendors)
        .filter(([vendor, rateValue]) => {
          return (
            rateValue !== null && rateValue !== 0 && vendor !== "BanffPay Rate"
...
export const sendRate = async () => {
  try {
    const mailList = [
      "dharold@bpay.africa",
      "mlawal@bpay.africa",
      "eamrovhe@bpay.africa",
      "osaliu@banffpay.com",
      "mebitanmi@banffpay.com",
      "eakinlua@bpay.africa",
      "cidefoh@banffpay.com",
    ...
    const pairsCombo = currencies.flatMap((from) =>
      currencies.map((to) => `${from}/${to}`)
    );
        ...
        (rowCurrency) => `
            <tr>
                ...
                ${currencies
                  .map((colCurrency) => {
                    const rate =
                      rowCurrency === colCurrency
                        ? 1
                        : exchangeRates[rowCurrency]?.[colCurrency] || "";
                    const bgColor =
                      rowCurrency === colCurrency ? "#FFFF00" : ""; // Highlight diagonal cells in yellow
                    // { console.log(typeof rate === 'number' && isFinite(rate) && rate.toFixed(2), rate) }
                    return `<td style="background-color: ${bgColor}; text-align: center;">${
          ...
          CountryList.findOneByCurrencyCode(rowCurrency)?.code
                ...
                ${sellCurrencies
                  .map((colCurrency) => {
                    ...
                    return `<td style="background-color: ${bgColor}; text-align: center;">${
                      typeof rate === "number" && isFinite(rate)
                        ? addCommasToNumber(+rate.toFixed(2))
                        : 0
                    }</td>`;
                  })
                  .join("")}
