...
export const getSourceAndDesCountries = async (): Promise<ICountryResponse> => {
  ...
  try {
    ...
    const [response1, response2] = await Promise.all([
      axios.post(
        `${process.env.PARTNER_REMITONE_BASE_URL}admin/remittance/get-source-countries`,
        payload
      ),
      axios.post(
        `${process.env.PARTNER_REMITONE_BASE_URL}admin/remittance/get-destination-countries`,
        payload
      ),
    ]);
    const data = {
      source: response1.data.data.result.countries.country,
      destination: response2.data.data.result.countries.country,
    };
...
export const updateSellRate = async (
  countryId: string,
  currency: string,
  ...
  try {
    const response = await axios.post(
      `${process.env.PARTNER_REMITONE_BASE_URL}admin/remittance/update-sell-rate`,
      {
        username,
        password,
        pin,
